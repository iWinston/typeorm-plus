...
describe("persistence > many-to-one relation", function() {
    ...
    it.skip("should remove exist post by cascades when category is saved without a post (post is set to null)", () => Promise.all(connections.map(async connection => {
        const post = new Post("Hello Post");
        await connection.manager.save(post);
